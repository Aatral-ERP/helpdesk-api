...
public class DealDeliveryChallan implements Cloneable {
    ...
    @Override
    public DealDeliveryChallan clone() throws CloneNotSupportedException {
        try {
            DealDeliveryChallan dcClone = (DealDeliveryChallan) super.clone();
            List<DealDCProducts> dcProducts = dcClone.getProducts().stream().map(_dcp -> {
                try {
                    return _dcp.clone();
            ...
            dcClone.setProducts(dcProducts);
