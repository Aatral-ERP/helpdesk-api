...
public class DealDAOImpl implements DealDAO {
    ...
    public Map<String, Object> getDeals(DealSearchRequest dealSearchReq) {
        ...
        try {
            String filterQuery = "", dealFilter = "", quoteFilter = "", poFilter = "", soFilter = "", proInvoiceFilter = "", invoiceFilter = "";
//
            ...
            if (dealSearchReq.getDealCreatedDateFrom() != null && dealSearchReq.getDealCreatedDateTo() != null) {
                dealFilter = dealFilter + " and d.createddatetime between '" + Util.sdfFormatter(dealSearchReq.getDealCreatedDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getDealCreatedDateTo()) + " 23:59:59'";
            }

            if (dealSearchReq.getDealModifiedDateFrom() != null && dealSearchReq.getDealModifiedDateTo() != null) {
                dealFilter = dealFilter + " and d.lastupdatedatetime between '" + Util.sdfFormatter(dealSearchReq.getDealModifiedDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getDealModifiedDateTo()) + " 23:59:59'";
            }
            ...
            if (dealSearchReq.getDealProducts() != null && dealSearchReq.getDealProducts().size() > 0) {
                ...
                dealFilter = dealFilter + " and d.id in ( select d.dealId from DealProducts dp where dp.productId in (" + productIds + "))";
            }
            ...
            if (dealSearchReq.getQuoteDateFrom() != null && dealSearchReq.getQuoteDateTo() != null) {
                quoteFilter = quoteFilter + " and dq.quoteDate between '" + Util.sdfFormatter(dealSearchReq.getQuoteDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getQuoteDateTo()) + "'";
            }
            if (dealSearchReq.getQuoteValidDateFrom() != null && dealSearchReq.getQuoteValidDateTo() != null) {
                quoteFilter = quoteFilter + " and dq.validUntil between '" + Util.sdfFormatter(dealSearchReq.getQuoteValidDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getQuoteValidDateTo()) + "'";
            }
            ...
            if (dealSearchReq.getPoDateFrom() != null && dealSearchReq.getPoDateTo() != null) {
                poFilter = poFilter + " and dpo.purchaseOrderDate between '" + Util.sdfFormatter(dealSearchReq.getPoDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getPoDateTo()) + "'";
            }
            if (dealSearchReq.getPoDueDateFrom() != null && dealSearchReq.getPoDueDateTo() != null) {
                poFilter = poFilter + " and dpo.dueDate between '" + Util.sdfFormatter(dealSearchReq.getPoDueDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getPoDueDateTo()) + "'";
            }
            ...
            if (dealSearchReq.getSoDueDateFrom() != null && dealSearchReq.getSoDueDateTo() != null) {
                soFilter = soFilter + " and dso.dueDate between '" + Util.sdfFormatter(dealSearchReq.getSoDueDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getPoDueDateTo()) + "'";
            }
            ...
            if (dealSearchReq.getProInvoiceDueDateFrom() != null && dealSearchReq.getProInvoiceDueDateTo() != null) {
                proInvoiceFilter = proInvoiceFilter + " and dpi.dueDate between '" + Util.sdfFormatter(dealSearchReq.getProInvoiceDueDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getProInvoiceDueDateTo()) + "'";
            }
            ...
            if (dealSearchReq.getInvoiceDateFrom() != null && dealSearchReq.getInvoiceDateTo() != null) {
                invoiceFilter = invoiceFilter + " and di.invoiceDate between '" + Util.sdfFormatter(dealSearchReq.getInvoiceDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getInvoiceDateTo()) + "'";
            }
            if (dealSearchReq.getInvoiceDueDateFrom() != null && dealSearchReq.getInvoiceDueDateTo() != null) {
                invoiceFilter = invoiceFilter + " and di.dueDate between '" + Util.sdfFormatter(dealSearchReq.getInvoiceDueDateFrom()) + "' and '" + Util.sdfFormatter(dealSearchReq.getInvoiceDueDateTo()) + "'";
            }
            ...
            if (!poFilter.isEmpty()) {
                filterQuery = filterQuery + " and d.id in (select dealId from DealPurchaseOrder dpo where 2>1 " + poFilter + ")";
            }
            ...
            if (!proInvoiceFilter.isEmpty()) {
                filterQuery = filterQuery + " and d.id in (select dealId from DealProformaInvoice dpi where 2>1 " + proInvoiceFilter + ")";
            }
        ...
        try {
            ...
            if (dealRepo.findById(dealId) != null) {
                ...
                if (invs.size() > 0) {
                    ...
                    List<Integer> invoiceIds = invs.stream().map(DealInvoice::getId).distinct().collect(Collectors.toList());
    ...
    public Map<String, Object> getAllNotes(Notes note) {
        ...
        try {
            ...
            if (notes.size() > 0) noteAttachments = noteAttachRepo.findByDealId(note.getDealId());
    ...
    public Map<String, Object> generateQuotationPDFTemplate1Lite(DealRequest dealReq) {
        ...
        try {
            ...
            if (dealReq.getDeal().getDealType().equalsIgnoreCase("AMC") && dealReq.getDeal().getAmcFromDate() != null && dealReq.getDeal().getAmcToDate() != null) {
                String subject = "Sub: " + dealReq.getDealQuotation().getSubject() + " <br>AMC Charges for the following items for the period of " + Util.sdfFormatter(dealReq.getDeal().getAmcFromDate(), "dd/MM/yyyy") + " to " + Util.sdfFormatter(dealReq.getDeal().getAmcToDate(), "dd/MM/yyyy");
            ...
            price_summary_label = price_summary_label + "Original Rate : <br>";
            price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getSubTotal()) + "<br>";

            if (dealReq.getDeal().getDiscount() > 0) {
                price_summary_label = price_summary_label + "Discount : <br>";
                price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getDiscount()) + "<br>";
            }

            price_summary_label = price_summary_label + "Taxable Amount : <br>";
            price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getSubTotal() - dealReq.getDeal().getDiscount()) + "<br>";

            if (dealReq.getDeal().getGstType().equalsIgnoreCase("IGST")) {
                price_summary_label = price_summary_label + "IGST : <br>";
                price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getTax()) + "<br>";
            } else {
                price_summary_label = price_summary_label + "CGST : <br>";
                price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getTax() / 2) + "<br>";
            ...
            if (dealReq.getDeal().getAdjustment() != 0.00) {
                ...
                price_summary_text = price_summary_text + "Rs." + String.format("%.2f", dealReq.getDeal().getAdjustment()) + "<br>";
            }
            ...
            List<Integer> partIds = dealReq.getDealProducts().stream().mapToInt(DealProducts::getPartId).distinct().boxed().sorted().collect(Collectors.toList());

            int sno = 1;
            for (int partId : partIds) {
                Map<String, String> data = new HashMap<>();

                List<DealProducts> dp = dealReq.getDealProducts().stream().filter(_dp -> _dp.getPartId() == partId).collect(Collectors.toList());

                data.put("sno", sno + ".");
                data.put("total", Util.decimalFormatter(dp.stream().map(prod -> (double) prod.getPrice() * prod.getQuantity()).mapToDouble(amount -> amount).sum()));
...
//		 Export the report to a PDF file.
    ...
    public Map<String, Object> generateQuotationPDFTemplate3Lite(DealRequest dealReq) {
        ...
        try {
            ...
            if (dealReq.getDeal().getDealType().equalsIgnoreCase("AMC") && dealReq.getDeal().getAmcFromDate() != null && dealReq.getDeal().getAmcToDate() != null) {
                String subject = "Sub: " + dealReq.getDealQuotation().getSubject() + " <br> Charges for the following items for the period of " + Util.sdfFormatter(dealReq.getDeal().getAmcFromDate(), "dd/MM/yyyy") + " to " + Util.sdfFormatter(dealReq.getDeal().getAmcToDate(), "dd/MM/yyyy");
                parameters.put("subject", subject);
            } else {
                parameters.put("subject", "Sub: " + dealReq.getDealQuotation().getSubject());
            }
...
//		 Export the report to a PDF file.
    ...
    public Map<String, Object> generateQuotationPDFTemplate1(DealRequest dealReq) {
        ...
        try {
            ...
            if (dealReq.getDeal().getGstType().equals("IGST")) {
                stream = this.getClass().getResourceAsStream("/reports/FinalTemplates/Template_1/Template_1_IGST.jrxml");
            } else {
                stream = this.getClass().getResourceAsStream("/reports/FinalTemplates/Template_1/Template_1_CGST_SGST.jrxml");
            }
...
//			parameters.put("quote_no", String.valueOf(dealReq.getDealQuotation().getQuoteNo()));
            ...
            deal_date_label = deal_date_label + "Valid Until : <br>";
            deal_date_text = deal_date_text + Util.sdfFormatter(dealReq.getDealQuotation().getValidUntil()) + "<br>";
            deal_date_label = deal_date_label + "Reference Number : <br>";
            deal_date_text = deal_date_text + String.valueOf(dealReq.getDealQuotation().getReferenceNo()) + "<br>";
            deal_date_label = deal_date_label + "Reference Date : <br>";
            deal_date_text = deal_date_text + Util.sdfFormatter(dealReq.getDealQuotation().getReferenceDate()) + "<br>";
            parameters.put("deal_date_label", deal_date_label);
            parameters.put("deal_date_text", deal_date_text);
            ...
            for (int partId : partIds) {
                ...
                if (partIds.size() > 1) {
                    ...
                    Double totalAmount = dealReq.getDealProducts().stream().filter(prod -> prod.getPartId() == partId).mapToDouble(DealProducts::getTotalAmount).sum();
...
//		 Export the report to a PDF file.
    ...
    public Map<String, Object> generateQuotationPDFTemplate3(DealRequest dealReq) {
        ...
        try {
            ...
            if (dealReq.getDeal().getGstType().equals("IGST")) {
                stream = this.getClass().getResourceAsStream("/reports/FinalTemplates/Template_3/Template_3_IGST.jrxml");
            } else {
                stream = this.getClass().getResourceAsStream("/reports/FinalTemplates/Template_3/Template_3_CGST_SGST.jrxml");
            }
...
//		 Export the report to a PDF file.
    ...
    public Map<String, Object> saveDealProformaInvoice(DealProformaInvoice dealProInvReq) {
        ...
        try {
            ...
            if (dealProInvReq.getId() == 0 && dpitemp != null) {
            ...
            } else if (dpitemp != null && dealProInvReq.getId() > 0 && dpitemp != null && dealProInvReq.getId() != dpitemp.getId()) {
                ...
                Query query = em.createQuery("UPDATE Deal d SET d.proformaInvoiceNo = :proformaInvoiceNo WHERE d.id = :id");
...
//			parameters.put("invoice_label", "PROFORMA INVOICE");
            ...
            deal_date_text = deal_date_text + String.valueOf(dealReq.getDealProformaInvoice().getProformaInvoiceNo()) + "<br>";
            deal_date_label = deal_date_label + "PI Date : <br>";
            deal_date_text = deal_date_text + Util.sdfFormatter(dealReq.getDealProformaInvoice().getInvoiceDate()) + "<br>";
            ...
            if (dealReq.getDealProformaInvoice().getPurchaseOrderNo() != null && dealReq.getDealProformaInvoice().getPurchaseOrderNo() != "") {
                deal_date_label = deal_date_label + "PO Number : <br>";
                deal_date_text = deal_date_text + String.valueOf(dealReq.getDealProformaInvoice().getPurchaseOrderNo()) + "<br>";
            }
            if (dealReq.getDealProformaInvoice().getSalesOrderNo() != null && dealReq.getDealProformaInvoice().getSalesOrderNo() != "") {
                deal_date_label = deal_date_label + "Sales Order No : <br>";
                deal_date_text = deal_date_text + String.valueOf(dealReq.getDealProformaInvoice().getSalesOrderNo()) + "<br>";
            }
            ...
            parameters.put("grand_total_text", "Grand Total : ");
            parameters.put("grand_total", "Rs." + Util.decimalFormatter((dealReq.getDeal().getGrandTotal() - dealReq.getDealProformaInvoice().getPaidAmount()) + dealReq.getDealProformaInvoice().getShippingCost()));
            ...
            if (dealReq.getDeal().getDealType().equalsIgnoreCase("AMC") && dealReq.getDeal().getAmcFromDate() != null && dealReq.getDeal().getAmcToDate() != null) {
                String subject = "Sub: " + dealReq.getDealProformaInvoice().getSubject() + " <br>AMC Charges for the following items for the period of " + Util.sdfFormatter(dealReq.getDeal().getAmcFromDate(), "dd/MM/yyyy") + " to " + Util.sdfFormatter(dealReq.getDeal().getAmcToDate(), "dd/MM/yyyy");
                parameters.put("subject", subject);
            } else {
            ...
            price_summary_text = price_summary_text + "Rs." + Util.decimalFormatter(dealReq.getDeal().getGrandTotal()) + "<br>";

            if (dealReq.getDealProformaInvoice().getPaidAmount() > 0) {
                price_summary_label = price_summary_label + "Paid Amount : <br>";
                price_summary_text = price_summary_text + "" + Util.decimalFormatter(dealReq.getDealProformaInvoice().getPaidAmount()) + "<br>";
            }

            if (dealReq.getDealProformaInvoice().getShippingCost() > 0) {
                price_summary_label = price_summary_label + "Shipment Amount : <br>";
                price_summary_text = price_summary_text + "" + Util.decimalFormatter(dealReq.getDealProformaInvoice().getShippingCost()) + "<br>";
            }
...
//				 Export the report to a RTF file.
    ...
    public Map<String, Object> generateProformaInvoicePDFTemplate2Lite(DealRequest dealReq) {
        ...
        try {
            ...
            if (dealReq.getDeal().getGstType().equals("IGST")) {
                stream = this.getClass().getResourceAsStream("/reports/ProformaInvoice/Proforma_Invoice_Template_2_IGST.jrxml");
            } else {
                stream = this.getClass().getResourceAsStream("/reports/ProformaInvoice/Proforma_Invoice_Template_2_CGST_SGST.jrxml");
            }
            ...
            parameters.put("balance_amount", "Rs." + Util.decimalFormatter((dealReq.getDeal().getGrandTotal() - dealReq.getDealProformaInvoice().getPaidAmount()) + dealReq.getDealProformaInvoice().getShippingCost()));
            parameters.put("amount_in_words", Util.EnglishNumberToWords((dealReq.getDeal().getGrandTotal() - dealReq.getDealProformaInvoice().getPaidAmount()) + dealReq.getDealProformaInvoice().getShippingCost()));
...
//				 Export the report to a RTF file.
    ...
    public Map<String, Object> generateProformaInvoicePDFTemplate3Lite(DealRequest dealReq) {
        ...
        try {
            ...
            parameters.put("deal_amount_text", "Rs." + Util.decimalFormatter((dealReq.getDeal().getGrandTotal() - dealReq.getDealProformaInvoice().getPaidAmount()) + dealReq.getDealProformaInvoice().getShippingCost()));
            parameters.put("amount_in_words", Util.EnglishNumberToWords((dealReq.getDeal().getGrandTotal() - dealReq.getDealProformaInvoice().getPaidAmount()) + dealReq.getDealProformaInvoice().getShippingCost()));
...
//			parameters.put("purchase_order_no", dealReq.getSalesOrder().getPurchaseOrderNo());
            ...
            if (dealReq.getSalesOrder().getPurchaseOrderNo() != null && dealReq.getSalesOrder().getPurchaseOrderNo() != "") {
                deal_date_label = deal_date_label + "PO Number : <br>";
    ...
    public Map<String, Object> saveDealPayments(DealPayments payment) {
        ...
        try {
            ...
            Query query = em.createQuery("UPDATE DealInvoice di SET di.paidAmount = :paidAmount WHERE di.dealId = :dealId");
    ...
    public Map<String, Object> createDealPaymentsReceipt1(DealRequest dealReq) {
        ...
        try {
            ...
            if (deal.getInvoiceNo() != null) parameters.put("bill_no", deal.getInvoiceNo());
            else parameters.put("bill_no", "");
...
//	}
    ...
    public Map<String, Object> saveDealDeliveryChallan(DealDeliveryChallan dc) {
        Map<String, Object> resp = new HashMap<>();
        DealDeliveryChallan finalDc = null;
        try {
            dc.getProducts().forEach(_dcp -> {
                _dcp.setInvoiceNo(dc.getInvoiceNo());
                _dcp.getDealDCProductRawMaterials().forEach(_dcpRM -> _dcpRM.setInvoiceNo(dc.getInvoiceNo()));
            });

            boolean newDC = dc.getId() == 0;
            final DealDeliveryChallan oldDC = newDC ? null : dcRepo.findById(dc.getId()).clone();

            finalDc = dcRepo.save(dc);

            if (newDC) {
                for (DealDCProducts dcp : dc.getProducts()) {
                    addStockEntryForNewDC(dc, dcp);
                }
            } else {

                Set<Integer> productIds = finalDc.getProducts().stream().map(DealDCProducts::getProductId).collect(Collectors.toSet());
                productIds.addAll(oldDC.getProducts().stream().map(DealDCProducts::getProductId).collect(Collectors.toSet()));

                addStockEntriesForUpdatedDCProducts(oldDC, finalDc, productIds);
                addStockEntriesForUpdatedDCProductsRawMaterial(oldDC, finalDc, productIds);
        ...
        resp.put("DealDeliveryChallan", finalDc);
        return resp;
    }

    private void addStockEntriesForUpdatedDCProductsRawMaterial(DealDeliveryChallan oldDC, DealDeliveryChallan finalDc, Set<Integer> productIds) {
        ...
        productIds.forEach(productId -> {
            ...
            rawProductIds.forEach(rawProductId -> {

                Optional<DealDCProductRawMaterial> dcpRM = dcpOptional.isPresent() ? dcpOptional.get().getDealDCProductRawMaterials().stream().filter(_dcpRM -> _dcpRM.getRawMaterialProductId() == rawProductId).findFirst() : Optional.empty();
                Optional<DealDCProductRawMaterial> OldDcpRM = OldDcpOptional.isPresent() ? OldDcpOptional.get().getDealDCProductRawMaterials().stream().filter(_dcpRM -> _dcpRM.getRawMaterialProductId() == rawProductId).findFirst() : Optional.empty();

                StockEntry se = new StockEntry();
                se.setProductId(rawProductId);
                se.setEntryDate(new Date());
                se.setEntryBy("--system-generated--");

                if (dcpRM.isPresent() && OldDcpRM.isPresent() && dcpRM.get().getQuantity() == OldDcpRM.get().getQuantity()) {
                    // Do Nothing , Because No Change in Quantity
                } else if (dcpRM.isPresent() && OldDcpRM.isPresent() && dcpRM.get().getQuantity() != OldDcpRM.get().getQuantity()) {
                    if (dcpRM.get().getQuantity() > OldDcpRM.get().getQuantity()) {
                        se.setQuantity(dcpRM.get().getQuantity() - OldDcpRM.get().getQuantity());
                        se.setType("Deduct");
                        se.setRemarks(SalesUtil.composeDCRawMaterialUpdatedStockEntryRemark(dcpRM.get(), finalDc));

                        agentService.addStockEntry(se);
                    } else if (OldDcpRM.get().getQuantity() > dcpRM.get().getQuantity()) {
                        se.setQuantity(OldDcpRM.get().getQuantity() - dcpRM.get().getQuantity());
                        se.setType("Credit");
                        se.setRemarks(SalesUtil.composeDCRawMaterialUpdatedStockEntryRemark(dcpRM.get(), finalDc));

                        agentService.addStockEntry(se);
                    }
                } else if (dcpRM.isPresent() && !OldDcpRM.isPresent()) {
                    se.setQuantity(dcpRM.get().getQuantity());
                    se.setType("Deduct");
                    se.setRemarks(SalesUtil.composeDCRawMaterialUpdatedStockEntryRemark(dcpRM.get(), finalDc));

                    agentService.addStockEntry(se);
                } else if (OldDcpRM.isPresent() && !dcpRM.isPresent()) {
                    se.setQuantity(OldDcpRM.get().getQuantity());
                    se.setType("Credit");
                    se.setRemarks(SalesUtil.composeDCRawMaterialUpdatedStockEntryRemark(OldDcpRM.get(), finalDc));
    ...
    private void addStockEntriesForUpdatedDCProducts(DealDeliveryChallan oldDC, DealDeliveryChallan finalDc, Set<Integer> productIds) {

        productIds.forEach(productId -> {
            Optional<DealDCProducts> dcp = finalDc.getProducts().stream().filter(_dcp -> _dcp.getProductId() == productId).findFirst();
            Optional<DealDCProducts> OldDcp = oldDC.getProducts().stream().filter(_dcp -> _dcp.getProductId() == productId).findFirst();

            StockEntry se = new StockEntry();
            se.setProductId(productId);
            se.setEntryDate(new Date());
            se.setEntryBy("--system-generated--");

            if (dcp.isPresent() && OldDcp.isPresent() && dcp.get().getQuantity() == OldDcp.get().getQuantity()) {
                // Do Nothing , Because No Change in Quantity
            } else if (dcp.isPresent() && OldDcp.isPresent() && dcp.get().getQuantity() != OldDcp.get().getQuantity()) {
                if (dcp.get().getQuantity() > OldDcp.get().getQuantity()) {
                    se.setQuantity(dcp.get().getQuantity() - OldDcp.get().getQuantity());
                    se.setType("Deduct");
                    se.setRemarks(SalesUtil.composeDCUpdatedStockEntryRemark(dcp.get(), finalDc));

                    agentService.addStockEntry(se);
                } else if (OldDcp.get().getQuantity() > dcp.get().getQuantity()) {
                    se.setQuantity(OldDcp.get().getQuantity() - dcp.get().getQuantity());
                    se.setType("Credit");
                    se.setRemarks(SalesUtil.composeDCUpdatedStockEntryRemark(dcp.get(), finalDc));

                    agentService.addStockEntry(se);
                }
            } else if (dcp.isPresent() && !OldDcp.isPresent()) {
                se.setQuantity(dcp.get().getQuantity());
                se.setType("Deduct");
                se.setRemarks(SalesUtil.composeDCUpdatedStockEntryRemark(dcp.get(), finalDc));

                agentService.addStockEntry(se);
            } else if (OldDcp.isPresent() && !dcp.isPresent()) {
                se.setQuantity(OldDcp.get().getQuantity());
                se.setType("Credit");
                se.setRemarks(SalesUtil.composeDCUpdatedStockEntryRemark(OldDcp.get(), finalDc));
    ...
    private void addStockEntryForNewDC(DealDeliveryChallan dc, DealDCProducts dcp) {
        // Adding Stock Entry for New DC Product
        StockEntry se = new StockEntry();
        ...
        se.setType("Deduct");
        se.setRemarks(SalesUtil.composeDCGeneratedStockEntryRemark(dcp, dc));

        agentService.addStockEntry(se);

        // Adding Stock Entry for each Raw Materials per DC Product
        dcp.getDealDCProductRawMaterials().forEach(_dcpRM -> {
            StockEntry seRM = new StockEntry();
            seRM.setProductId(_dcpRM.getRawMaterialProductId());
            seRM.setEntryDate(new Date());
            seRM.setEntryBy("--system-generated--");
            seRM.setQuantity(_dcpRM.getQuantity());
            seRM.setType("Deduct");
            seRM.setRemarks(SalesUtil.composeDCRawMaterialGeneratedStockEntryRemark(_dcpRM, dc));

            agentService.addStockEntry(seRM);
        });
    }
    ...
    public Map<String, Object> deleteDealDeliveryChallan(DealDeliveryChallan dc) {
        ...
        try {
            ...
            for (DealDCProducts dcp : dc.getProducts()) {
                ...
                se.setQuantity(dcp.getQuantity());
                se.setType("Credit");
                se.setRemarks(SalesUtil.composeDCDeletedStockEntryRemark(dcp, dc));
                ...
                dcp.getDealDCProductRawMaterials().forEach(_dcpRM -> {
                    ...
                    seRM.setQuantity(_dcpRM.getQuantity());
                    seRM.setType("Credit");
                    seRM.setRemarks(SalesUtil.composeDCRawMaterialDeletedStockEntryRemark(_dcpRM, dc));
    ...
    public Map<String, Object> getPendingDCQuantityProducts(DealRequest dealReq) {
        ...
        Map<String, Object> resp = new HashMap<>();
        List<DealProducts> remainingDealProducts = new ArrayList<>();
        ...
        try {
            ...
            if (dealReq.getDeal().getId() > 0) {
                ...
                for (DealProducts _dp : __dealProducts) {
                    DealProducts dpClone = _dp.clone();
                    for (DealDCProducts dcp : dcps) {
                        if (dpClone.getProductId() == dcp.getProductId())
                            dpClone.setQuantity(dpClone.getQuantity() - dcp.getQuantity());
                    }
                    if (dpClone.getQuantity() > 0) remainingDealProducts.add(dpClone);
                }
            }
        ...
        resp.put("ReminingDealProducts", remainingDealProducts);
        return resp;
    ...
    public Map<String, Object> getDealQuotationsReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getDealCreatedDateFrom() != null && req.getDealCreatedDateTo() != null) {
                dealFilter = dealFilter + " and d.createddatetime between '" + Util.sdfFormatter(req.getDealCreatedDateFrom()) + "' and '" + Util.sdfFormatter(req.getDealCreatedDateTo()) + " 23:59:59'";
            }

            if (req.getDealModifiedDateFrom() != null && req.getDealModifiedDateTo() != null) {
                dealFilter = dealFilter + " and d.lastupdatedatetime between '" + Util.sdfFormatter(req.getDealModifiedDateFrom()) + "' and '" + Util.sdfFormatter(req.getDealModifiedDateTo()) + " 23:59:59'";
            }
            ...
            if (req.getDealProducts() != null && req.getDealProducts().size() > 0) {
                ...
                dealFilter = dealFilter + " and d.id in ( select dp.dealId from DealProducts dp where dp.productId in (" + productIds + "))";
            }
            ...
            if (req.getQuoteDateFrom() != null && req.getQuoteDateTo() != null) {
                dealFilter = dealFilter + " and dq.quoteDate between '" + Util.sdfFormatter(req.getQuoteDateFrom()) + "' and '" + Util.sdfFormatter(req.getQuoteDateTo()) + "'";
            }
            if (req.getQuoteValidDateFrom() != null && req.getQuoteValidDateTo() != null) {
                dealFilter = dealFilter + " and dq.validUntil between '" + Util.sdfFormatter(req.getQuoteValidDateFrom()) + "' and '" + Util.sdfFormatter(req.getQuoteValidDateTo()) + "'";
            }

            /* Deals Quotation Filter ends */

            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dq) from DealQuotation dq " + "left join fetch Deal d on (dq.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealPurchaseOrdersReport(DealSearchRequest req) {
        ...
        try {
            ...
            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dpo) from DealPurchaseOrder dpo " + "left join fetch Deal d on (dpo.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealProjectImplementationsReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getDate() != null || req.getDate() != "" || req.getDate() != "0" || !req.getDate().isEmpty()) {
                dealFilter = dealFilter + "AND dpim.createddatetime BETWEEN '" + date.minusMonths(Integer.parseInt(req.getDate())) + "' AND '" + date + "' ";
            ...
            if (req.getFromDate() != null || req.getToDate() != null) {
                ...
                dealFilter = dealFilter + "AND dpim.createddatetime BETWEEN '" + sdf1.format(req.getFromDate()).toString() + "' AND '" + sdf1.format(req.getToDate()).toString() + "' ";
            }

            /* Deals Project Implementations Filter ends */

            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dpim) from DealProjectImplementation dpim " + "left join fetch Deal d on (dpim.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealSalesOrdersReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getSoDueDateFrom() != null && req.getSoDueDateTo() != null) {
                dealFilter = dealFilter + " and dso.dueDate between '" + Util.sdfFormatter(req.getSoDueDateFrom()) + "' and '" + Util.sdfFormatter(req.getSoDueDateTo()) + "'";
            }
            ...
            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dso) from DealSalesOrder dso " + "left join fetch Deal d on (dso.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealProformaInvoicesReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getProInvoiceDateFrom() != null && req.getProInvoiceDateTo() != null) {
                dealFilter = dealFilter + " and dpi.invoiceDate between '" + Util.sdfFormatter(req.getProInvoiceDateFrom()) + "' and '" + Util.sdfFormatter(req.getProInvoiceDateTo()) + "'";
            }
            if (req.getProInvoiceDueDateFrom() != null && req.getProInvoiceDueDateTo() != null) {
                dealFilter = dealFilter + " and dpi.dueDate between '" + Util.sdfFormatter(req.getProInvoiceDueDateFrom()) + "' and '" + Util.sdfFormatter(req.getProInvoiceDueDateTo()) + "'";
            }

            /* Deals Proforma Invoice Filter ends */

            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dpi) from DealProformaInvoice dpi " + "left join fetch Deal d on (dpi.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealInvoicesReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getInvoiceDateFrom() != null && req.getInvoiceDateTo() != null) {
                dealFilter = dealFilter + " and di.invoiceDate between '" + Util.sdfFormatter(req.getInvoiceDateFrom()) + "' and '" + Util.sdfFormatter(req.getInvoiceDateTo()) + "'";
            }
            if (req.getInvoiceDueDateFrom() != null && req.getInvoiceDueDateTo() != null) {
                dealFilter = dealFilter + " and di.dueDate between '" + Util.sdfFormatter(req.getInvoiceDueDateFrom()) + "' and '" + Util.sdfFormatter(req.getInvoiceDueDateTo()) + "'";
            }
            ...
            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , di) from DealInvoice di left join fetch Deal d on (di.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealDeliveryChallansReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getDealProducts() != null && req.getDealProducts().size() > 0) {
                ...
                dealFilter = dealFilter + " and dc.id in ( select dc.dealId from DealDCProducts dcp where dcp.productId in (" + productIds + "))";
            }
            if (req.getDealType() != null && req.getDealType() != "") {
                dealFilter = dealFilter + " and d.dealType = '" + req.getDealType() + "'";
            }

            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dc) from DealDeliveryChallan dc " + "left join fetch Deal d on (dc.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealPaymentsReport(DealSearchRequest req) {
        ...
        try {
            ...
            if (req.getPaymentDateFrom() != null && req.getPaymentDateTo() != null) {
                dealFilter = dealFilter + " and dp.paymentDate between '" + Util.sdfFormatter(req.getPaymentDateFrom()) + "' and '" + Util.sdfFormatter(req.getPaymentDateTo()) + "'";
            }
            ...
            Query query = em.createQuery("select new com.autolib.helpdesk.Sales.model.DealResponse(d , dp) from DealPayments dp " + "left join fetch Deal d on (dp.dealId = d.id) where 2 > 1 " + dealFilter, DealResponse.class);
    ...
    public Map<String, Object> getDealProjectImplementations(ProjectImplemantationRequest projectReq) {
        ...
        try {
            ...
            dpims = dpimRepo.findMyAssignedProjectImplementations(projectReq.getMailId(), projectReq.getFromDate(), projectReq.getToDate());

            List<Integer> dealIds = dpims.stream().map(DealProjectImplementation::getDealId).collect(Collectors.toList());
            ...
            dpims.parallelStream().forEach(dpim -> {
                ...
                List<DealProducts> dps = dealProducts.stream().filter(dp -> dp.getDealId() == dpim.getDealId()).collect(Collectors.toList());
    ...
    void sendDealProjectImplementationMailAction(DealProjectImplementation dpim) {
        ...
        try {
            ...
            if (dpim.getMailAction().equalsIgnoreCase("assignManufacturingToAgent")) {
                ...
                emailModel.setMailSub("Manufacturing Marked As Completed , Needs Approval - " + deal.getInstitute().getInstituteName());
                ...
                emailModel.setMailSub("Delivery Marked As Completed, Needs Approval - " + deal.getInstitute().getInstituteName());
                ...
                emailModel.setMailSub("Installation Marked As Completed , Needs Approval - " + deal.getInstitute().getInstituteName());
...
//					jdbcTemp.update(DBQueryUtil.Insert_Payment, payParam);
    ...
    public Map<String, Object> generateSalesOrderPDF3(DealRequest dealReq) {
        ...
        try {
            ...
            deal_date_text = deal_date_text + String.valueOf(dealReq.getDealQuotation().getQuoteNo()) + "<br>";
            deal_date_label = deal_date_label + "Due Date  : <br>";
            deal_date_text = deal_date_text + String.valueOf(Util.sdfFormatter(dealReq.getSalesOrder().getDueDate())) + "<br>";
...
//			parameters.put("purchase_order_no", String.valueOf(dealReq.getDealProformaInvoice().getPurchaseOrderNo()));
            ...
            deal_date_text = deal_date_text + String.valueOf(Util.sdfFormatter(dealReq.getDealProformaInvoice().getInvoiceDate())) + "<br>";
