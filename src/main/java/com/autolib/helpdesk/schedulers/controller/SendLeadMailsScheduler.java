        ...
        try {
            ...
            if (_lead.getFiles() != null && !_lead.getFiles().isEmpty()) {
                for (String fileName : Arrays.asList(_lead.getFiles().split(";"))) {
                    s3StorageService.pullFileFromS3ToLocal(S3Directories.LeadFiles + _lead.getId() + "/" + fileName,
                            LocalDirectory.LeadFiles + _lead.getId() + "/", fileName);
                    directory = new File(LocalDirectory.LeadFiles + _lead.getId() + "/" + fileName);
                    attachs.add(directory.getAbsolutePath());
                }
            }
